...
import { usePreferences, type Currency } from '@/context/PreferencesContext'
...
const OPTIONS: { value: Currency; label: string; sub: string }[] = [
  ...
  { value: 'IDR', label: 'IDR', sub: 'Indonesian Rupiah' },
]

export default function SettingsPreferencesCurrency() {
          ...
          {OPTIONS.map((opt) => (
            ...
            >
              <div className="flex flex-col gap-0.5">
                <p
                  className={`text-sm ${currency === opt.value ? 'font-medium text-primary' : 'text-foreground'}`}
                >
                  {opt.label}
                </p>
                <p className="text-xs text-muted-foreground">{opt.sub}</p>
              </div>
              {currency === opt.value && <Check size={16} className="text-primary" />}
            </button>
          ))}
        </div>
      </div>
